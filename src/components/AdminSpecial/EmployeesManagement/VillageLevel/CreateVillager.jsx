...
const CreateVillager = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('https://kernn.azurewebsites.net/api/create_villager', villagerData);
      toast({
        title: "Villager created successfully.",
  ...
  return (
    <Box p={4}>
      ...
      <Button colorScheme="teal" onClick={handleSubmit}>Create Villager</Button>
    </Box>
  );
};
